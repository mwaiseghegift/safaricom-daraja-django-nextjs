...
IMSI in full means International Mobile Subscriber Identity. The IMSI API allows users to query the network age of a Safaricom phone number. The API acts as a security enhancement thus preventing fraud cases. Partners will be able to query phone number network registration date, hashed IMSI number and the last swap date.

Partners will be able to get:
...
Organization sends an API request to Daraja with the customer's phone number
Daraja authenticates the API call and forwards the request to M-PESA
M-PESA returns the hashed IMSI, Age on network and last swap date to Daraja which forwards the response to organization system
...
| Sandbox
| Testing environment.
| [https://sandbox.safaricom.co.ke/imsi/v1/checkATI](https://sandbox.safaricom.co.ke/imsi/v1/checkATI)

| Production
| Live environment for real transactions.
| [https://api.safaricom.co.ke/imsi/v1/checkATI](https://api.safaricom.co.ke/imsi/v1/checkATI)
...
![imsi-sequence-diagram!](/docs/images/imsi-sequence-diagram.png)
...
| Customer Number
| The customer MSISDN whose network age is to be determined
| Numeric
...
| responseCode
| A Numeric status code that indicates the status of the transaction submission. 200 means successful submission
| Numeric
| 200
...
| imsi
| This is the hashed number representing the international mobile subscriber identity i.e imsi
| Numerical
| 4213215110021315

| lastSwapDate
| This is a string representing the date in which the mobile number was last swapped. If the specified SIM was swapped more than 3 months ago, the API returns a default date of 01-01-1900
| String
| 01-01-1900 00:00

| msisdnRegistrationDate
| Represents the MSISDN registration date on the network or a message depicting that the MSISDN registration is more than one year old.
| String
| 2019-01-12
...
Sandbox: [https://sandbox.safaricom.co.ke/mpesa/transactionstatus/v1/query](https://sandbox.safaricom.co.ke/mpesa/transactionstatus/v1/query)
Production: [https://sandbox.safaricom.co.ke/mpesa/transactionstatus/v1/query](https://sandbox.safaricom.co.ke/mpesa/transactionstatus/v1/query)
...
**What is the IMSI API?**

Description: The IMSI API allows users to query the network age of an MSISDN. The API acts as a security enhancement, preventing fraud cases. Partners can query the MSISDN network registration date and the last swap date.

**How does the IMSI API work?**

Sequence Diagram: The sequence diagram illustrates the interaction between the client application and the IMSI API.

Organization -> Payment Gateway: The API integrates with the organization's payment gateway to ensure secure transactions.
...
These FAQs should help you understand and utilize the IMSI API effectively. If you have more questions, feel free to reach out to the support team: [apisupport@safaricom.co.ke](mailto:apisupport@safaricom.co.ke).
