...
| Sandbox
| Testing environment.
| [https://sandbox.safaricom.co.ke/mpesa/reversal/v1/request](https://sandbox.safaricom.co.ke/mpesa/reversal/v1/request)

| Production
| Live environment for real transactions.
| [https://api.safaricom.co.ke/mpesa/reversal/v1/request](https://api.safaricom.co.ke/mpesa/reversal/v1/request)

## Integration Steps

### Sequence Diagram

![reversal-sequence-diagram!](/docs/images/reversal-sequence-diagram.png)
...
| URL
| Yes
| [https://mydomain.com/reversal/timedout](https://mydomain.com/reversal/timedout)

| ResultURL
| URL for result notification
| URL
| Yes
| [https://mydomain.com/reversal/result](https://mydomain.com/reversal/result)
...
| Result
| Root parameter for the result message
| JSON Object
| No
| `{}`

| ResultType
| Status code (usually 0)
| Numeric
| No
| 0
...
| String
| No
| AG_20210707_20106f7a33

| TransactionID
| M-PESA Receipt Number for the reversal request
| String
| No
| SKE52PAWR9
...
| DebitAccountBalance
| Account balances (format: `Account Type
| Currency
| ...`)
| String
...
| TransCompletedTime
| Completion time (YYYYMMDDhhmmss)
| String
| Yes
| 20211114132711
...
| 0
| The service request is processed successfully
| Request processed successfully on M-PESA.

| R000002
| The OriginalTransactionID is invalid
| The TransactionID provided is invalid or does not exist on M-PESA.

| R000001
| The transaction has already been reversed
| The TransactionID provided is already reversed.

| 11
| The DebitParty is in an invalid state
| The organization/short code account is not active.

| 21
| The initiator is not allowed to initiate
| API user lacks Org Reversals Initiator API role.
...
| 2006
| Declined due to account rule
| Organization/short code account is not active.

| 2028
| Not permitted according to product assignment
| Short code has no permission to perform the request.

| 8006
| The security credential is locked
| API user password is locked.

| 1
| The balance is insufficient
| Short code does not have enough money to complete the request.
...
| requestId
| Unique identifier assigned by API
| 30764-19833054-1
| String
...
| errorMessage
| Descriptive message of failure
| Bad Request - Invalid TransactionID
| String
...
| Regenerate a new access token and use it before expiry.
| 404
...
| Avoid sending multiple requests that violate API TPS limit.
| 500

| 500.003.03
| Quota Violation
| Avoid sending multiple requests that violate API requests limit.
| 500
